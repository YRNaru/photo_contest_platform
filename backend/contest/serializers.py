...
class EntryListSerializer(serializers.ModelSerializer):
    ...
    contest_slug = serializers.CharField(
        source='contest.slug', read_only=True
    )
    contest_title = serializers.CharField(
        source='contest.title', read_only=True
    )

    class Meta:
        model = Entry
        fields = (
            'id', 'contest', 'contest_slug', 'contest_title',
            'author', 'title', 'description', 'tags',
            'created_at', 'vote_count', 'view_count',
            'thumbnail', 'approved'
        )
        read_only_fields = (
            'id', 'created_at', 'vote_count',
            'view_count', 'approved'
        )
...
class EntryCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if contest.max_entries_per_user > 0:
            ...
            if user_entries >= contest.max_entries_per_user:
                msg = (
                    f'このコンテストへの応募は最大'
                    f'{contest.max_entries_per_user}件までです。'
                )
        ...
        if max_images > 0 and len(images) > max_images:
            raise serializers.ValidationError(
                f'画像は最大{max_images}枚までアップロードできます。'
            )
        ...
        for image in images:
            ...
            existing_image = EntryImage.objects.filter(
    ...
    def create(self, validated_data):
        ...
        auto_approve = (
            contest.auto_approve_entries if contest else settings.DEBUG
        )
...
class VoteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # 最大投票数チェック
        max_votes = (
            category.get_max_votes()
            if category else contest.max_votes_per_judge
        )
