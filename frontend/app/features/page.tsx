...
function FeatureCard({ emoji, title, description, gradient }: FeatureCardProps) {
  return (
    <div className={`group p-6 ${gradient} rounded-xl border-2 hover:scale-105 transition-all duration-300 hover:shadow-xl transform-gpu`}>
      <div className="text-4xl mb-4 group-hover:scale-125 transition-transform duration-300">{emoji}</div>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-6xl">
      ...
      <FeatureSection emoji="🐦" title="Twitterとの連携">
        ...
        <SubFeature
          ...
          items={[
            "ツイートのURL、投稿者の情報、写真データなどを自動で保存します"
          ]}
        />
      </FeatureSection>
      ...
      <FeatureSection emoji="📊" title="コンテストの運営">
        ...
        <SubFeature
          title="コンテストの進行状況"
          items={[
            "システムが自動で判断します: 開催前、投稿受付中、投票期間中、審査中、終了"
          ]}
        />
      </FeatureSection>
      ...
      <FeatureSection emoji="🖼️" title="写真の処理">
        ...
        <SubFeature
          title="画像の管理"
          items={[
            "アップロードされた写真を整理し、不要なファイルを削除できます"
          ]}
        />
      </FeatureSection>
