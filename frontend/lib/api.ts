...
export const contestApi = {
  ...
  // コンテスト更新（作成者または管理者）
  updateContest: (slug: string, data: FormData) => api.patch(`/contests/${slug}/`, data),
...
export const entryApi = {
  ...
  // 通報
  flag: (id: string, reason: string) => api.post(`/entries/${id}/flag/`, { reason }),
...
export const authApi = {
  // Google OAuth
  googleLogin: (credential: string) => api.post('/auth/google/', { id_token: credential }),
...
export const voteApi = {
  ...
  // 投票作成
  createVote: (data: { entry: string; category?: number | null }) => api.post('/votes/', data),
