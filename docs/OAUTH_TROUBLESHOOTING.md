...
本番バックエンドのログ（journalctl / Docker 等）で以下を確認：
...
1. `https://api.example.com/admin/` にアクセス
2. スーパーユーザーでログイン
...
1. バックエンド・フロントを再ビルドし、プロセスを再起動する
...
**対処法**:
1. 本番の環境変数で `FRONTEND_URL` を確認
2. 正しい値: `https://www.example.com`
3. バックエンドを再デプロイ
...
**対処法**:
1. 本番の環境変数で `CORS_ALLOWED_ORIGINS` を確認
2. フロントエンドのURLを含める: `https://www.example.com`
3. バックエンドを再デプロイ
