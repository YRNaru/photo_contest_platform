...
### 開発者向け
- **MySQL接続**（ホストから Docker の DB へ）:
  ```bash
  mysql -h 127.0.0.1 -P 13306 -u contestuser -p contest
  ```
- **Redis接続**:
  ```bash
...
### データベース接続（ローカル開発時）
```bash
# Docker を使わずホストの MySQL に接続する場合（例）
DATABASE_URL=mysql://contestuser:contestpass@localhost:3306/contest

# Compose の DB をホストから参照する場合（docker-compose.yml の公開ポート）
DATABASE_URL=mysql://contestuser:contestpass@127.0.0.1:13306/contest
```
