...
### 開発者向け
- **PostgreSQL接続**（ホストから Docker の DB へ）:
  ```bash
  PGPASSWORD=contestpass psql -h 127.0.0.1 -p 15432 -U contestuser -d contest
  ```
- **Redis接続**: 
  ```bash
...
### データベース接続（ローカル開発時）
```bash
# Docker を使わずホストの PostgreSQL に接続する場合（例）
DATABASE_URL=postgresql://contestuser:contestpass@localhost:5432/contest

# Compose の DB をホストから参照する場合（docker-compose.yml の公開ポート）
DATABASE_URL=postgresql://contestuser:contestpass@127.0.0.1:15432/contest
```
